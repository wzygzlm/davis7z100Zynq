 ...
 *			reset when USB reset is asserted (CR 627547).
 *
 * </pre>
 *****************************************************************************/
/***************************** Include Files *********************************/
...
void InitUsbInterface(XUsb * InstancePtr)
{
	...
	regvalue = XUsb_ReadReg(InstancePtr->Config.BaseAddress,
					 InstancePtr->EndPointOffset[1]);
	regvalue = regvalue | (2 << 13);

	XUsb_WriteReg(InstancePtr->Config.BaseAddress,
				InstancePtr->EndPointOffset[1], regvalue);
